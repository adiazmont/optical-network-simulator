...
class Network():
    ...
    def transmit(self, src_node, dst_node, route='auto', spectrum_band='C', channel_no=1, channels='auto', launch_power=-2, bandwidth=12.5*(10E9), grid=0.4*(10E-9)):
        if route=='auto':
    ...
    def findPath(self, src_node, dst_node, path=[]):
        ...
        while current_node != dst_node:
            ...
            for node_to_link_relation in destinations:
                ...
                length_of_link = link_to_next_node.getLinkLength()
        ...
        for link in sorted(self.links):
            ...
            for span_obj in sorted(self.links[link]):
                ...
                if amplifier is not None:
                    link_string_representation += " ---" + str(span_length) + "km---" + edfa + str(amplifier.target_gain) + "dB" 
                else:
