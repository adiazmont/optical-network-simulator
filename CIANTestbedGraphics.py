...
class CIANTestbed:
    ...
    def __init__(self):
        ...
        self.net.addSpanToLink(l1, span_link1, Amplifier(target_gain=fibre_attenuation * span_link1.length,
                                                         wavelengthDependentGainId='wdg1'))
        edfa_gains.append(fibre_attenuation * span_link1.length)
        self.net.addSpanToLink(l2, span_link2, Amplifier(target_gain=fibre_attenuation * span_link2.length,
                                                         wavelengthDependentGainId='wdg1'))
        edfa_gains.append(fibre_attenuation * span_link2.length)
        self.net.addSpanToLink(l5, span_link5, Amplifier(target_gain=fibre_attenuation * span_link5.length,
                                                         wavelengthDependentGainId='wdg1'))
        edfa_gains.append(fibre_attenuation * span_link5.length)
        self.net.addSpanToLink(l6, span_link6, Amplifier(target_gain=fibre_attenuation * span_link6.length,
                                                         wavelengthDependentGainId='wdg1'))
        edfa_gains.append(fibre_attenuation * span_link6.length)
        self.net.addSpanToLink(l7, span_link7, Amplifier(target_gain=fibre_attenuation * span_link7.length,
                                                         wavelengthDependentGainId='wdg1'))
        ...
        print("OSNR of channel %s (nm) is %s dB at span %s." %(str(channel), str(self.net.monitor(l1, span_link1, 82)), span_link1.span_id))
        osnr_values.append(self.net.monitor(l1, span_link1, 82))
        spans_length.append(span_link1.length)
        print("OSNR of channel %s (nm) is %s dB at span %s." %(str(channel), str(self.net.monitor(l2, span_link2, 82)), span_link2.span_id))
        osnr_values.append(self.net.monitor(l2, span_link2, 82))
        spans_length.append(span_link2.length)
        print("OSNR of channel %s (nm) is %s dB at span %s." %(str(channel), str(self.net.monitor(l5, span_link5, 82)), span_link5.span_id))
        osnr_values.append(self.net.monitor(l5, span_link5, 82))
        spans_length.append(span_link5.length)
        print("OSNR of channel %s (nm) is %s dB at span %s." %(str(channel), str(self.net.monitor(l6, span_link6, 82)), span_link6.span_id))
